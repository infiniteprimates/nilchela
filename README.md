...
In the nilchela container this is not required — shims already surface `[env]` to child processes, and the entrypoint sets `PATH` via `mise env` — but it's the mechanism if you want to source everything dynamically instead of relying on the entrypoint-provided `PATH`.
