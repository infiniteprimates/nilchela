...
to child processes, and the entrypoint sets `PATH` via `mise env` — but it is
the lever if you want to source everything dynamically instead of relying on
the entrypoint-provided `PATH` and configured `[env]`.
